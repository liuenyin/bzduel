...
export function renderAutochess(container, data = {}) {
  ...
  function render() {
    ...
    container.innerHTML = `
      <div class="ac-page">
        <!-- 顶部状态栏 -->
        <div class="ac-topbar">
          <div class="ac-stat"><span class="ac-label">💰</span><span class="ac-val" id="ac-gold">${run.gold}</span></div>
          <div class="ac-stat"><span class="ac-label">📊 Lv.${run.level}</span><span class="ac-val">${run.xp}/${run.xpToNext || '∞'}</span></div>
          <div class="ac-stat"><span class="ac-label">❤️</span><span class="ac-val">${run.commanderHP}/${run.maxCommanderHP}</span></div>
          <div class="ac-stat"><span class="ac-label">💹</span><span class="ac-val">+${run.interest || 0}</span></div>
        ...
        <div class="ac-board">
          <div class="ac-hex-ring">
            ${HEX_SUBJECTS.map((subj, i) => {
              ...
              return `
                <div class="ac-hex-slot" data-slot="${subj}" data-index="${i}" style="--hex-i:${i}">
          ...
          <div class="ac-core-slot" data-slot="core">
            ${run.board.core ? (() => {
              ...
              `;
            })() : `<div class="ac-core-empty">拖入<br>主C</div>`}
          </div>
        </div>
        ...
        <div class="ac-actions">
          ...
          ` : run.phase === 'event' ? `
            <button id="ac-btn-invest" class="btn btn-primary">📈 投资策略</button>
            <button id="ac-btn-goldmine" class="btn btn-success">💰 打劫金矿</button>
          ` : run.phase === 'victory' ? `
        ...
        <!-- 备战席 -->
        <div class="ac-bench">
          <h3>📦 备战席 (${run.bench.length})</h3>
          <div class="ac-bench-list">
            ${run.bench.map((entry, i) => {
              const c = AC_CHAR_MAP[entry.charId];
              return `
                <div class="ac-bench-item" data-bench="${i}">
                  <span class="ac-char-name">${c?.name || '?'}</span>
                  <span class="ac-char-star">${'★'.repeat(entry.star)}${'☆'.repeat(3 - entry.star)}</span>
                  <span class="ac-char-cost">${c?.cost || '?'}费</span>
                  <div class="ac-bench-actions">
                    <button class="btn-sm ac-place-core" data-idx="${i}">→阵眼</button>
                    <select class="ac-place-hex" data-idx="${i}">
                      <option value="">→辅阵…</option>
                      ${HEX_SUBJECTS.map(s => `<option value="${s}">${SUBJECT_LABELS[s]}</option>`).join('')}
                    </select>
                    <button class="btn-sm ac-sell" data-idx="${i}">卖</button>
                  </div>
                </div>
              `;
            }).join('')}
  ...
  function bindEvents() {
    ...
    // 放入阵眼
    document.querySelectorAll('.ac-place-core').forEach(el => {
      el.addEventListener('click', () => {
        gameSocket.emit('ac_place', { benchIndex: parseInt(el.dataset.idx), slot: 'core' });
      });
    });

    // 放入辅阵
    document.querySelectorAll('.ac-place-hex').forEach(el => {
      el.addEventListener('change', () => {
        if (!el.value) return;
        gameSocket.emit('ac_place', { benchIndex: parseInt(el.dataset.idx), slot: el.value });
        el.value = '';
      });
    });

    // 卖出
    document.querySelectorAll('.ac-sell').forEach(el => {
      el.addEventListener('click', () => {
    ...
    // 从棋盘移回
    document.querySelectorAll('.ac-hex-slot').forEach(el => {
      el.addEventListener('click', () => {
        const slot = el.dataset.slot;
        if (run?.board?.hexSlots[slot]) {
          gameSocket.emit('ac_remove', { slot });
        }
      });
    });
    document.querySelector('.ac-core-slot')?.addEventListener('click', () => {
      if (run?.board?.core) {
        gameSocket.emit('ac_remove', { slot: 'core' });
      }
    });
  ...
  gameSocket.on('ac_star_up', ({ charId, fromStar, toStar }) => {
    ...
    showToast(`⭐ ${name} 升到 ${'★'.repeat(toStar)} 了!`);
  });
